...
app = FastAPI(
    ...
    description="Generate Google Calendar links from event text input",
    version="1.0.0"
)
...
def generate_calendar_link(payload: EventText):
    """
    Generate a Google Calendar link from event text input.
    ...
    try:
        ...
        if not match:
            raise ValueError("Date format not recognized. Expected format: 'Month DD, YYYY H:MM AM/PM EST'")
    ...
    # Build Google Calendar link with proper URL encoding
    base_url = "https://calendar.google.com/calendar/render?action=TEMPLATE"
    encoded_event_name = urllib.parse.quote_plus(event_name)
    params = f"&text={encoded_event_name}&dates={start_str}/{end_str}"
